 ...
 */
{
   ...
   void BI_zMPI_sum(void *, void *, int *, BI_MPI_Datatype *);
/*
 ...
 */
   ...
   case ' ':         /* use MPI's reduction by default */
      length = 1;
      BI_MPI_Op_create(BI_zMPI_sum, length, &BlacComb, ierr);
      if (dest != -1)
      {
         BI_MPI_Reduce(bp->Buff, bp2->Buff, bp->N, bp->dtype, BlacComb,
                       dest, ctxt->scp->comm, ierr);
