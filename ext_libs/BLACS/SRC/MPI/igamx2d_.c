 ...
 */
{
   ...
   void BI_iMPI_amx(void *, void *, int *, BI_MPI_Datatype *);
   void BI_iMPI_amx2(void *, void *, int *, BI_MPI_Datatype *);
/*
 ...
 */
   ...
   case ' ':         /* use MPI's reduction by default */
      ...
      if (Mpval(ldia) == -1)
      {
         BI_MPI_Op_create(BI_iMPI_amx2, i, &BlacComb, ierr);
      }
      else
      {
         BI_MPI_Op_create(BI_iMPI_amx, i, &BlacComb, ierr);
